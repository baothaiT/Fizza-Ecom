	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#pizza-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#pizza-contact').find('input,textarea').attr({value:''});
				jQuery('#pizza-contact').css({opacity:1});
				jQuery('#pizza-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#pizza-contact').animate({opacity:0.3});
		jQuery('#pizza-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#pizza-contact').find('input,textarea').css('border','none');
